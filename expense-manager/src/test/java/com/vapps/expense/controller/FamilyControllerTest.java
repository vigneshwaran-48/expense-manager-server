...
public class FamilyControllerTest {
	...
	public void shouldRejectJoinRequest() throws Exception {
		...
		MvcResult mvcResult = mockMvc.perform(post(UriComponentsBuilder.fromPath(Endpoints.FAMILY_JOIN_REQUEST)
				...
				.andReturn();
		JoinRequestResponse response = objectMapper.readValue(mvcResult.getResponse().getContentAsString(),
		...
		mvcResult = mockMvc.perform(post(UriComponentsBuilder.fromPath(Endpoints.FAMILY_REJECT_JOIN_REQUEST)
						.buildAndExpand(familyId, response.getRequest().getId()).toUriString()).with(
						oidcLogin().oidcUser(getOidcUser(USER_ID,
	...
	public void shouldListJoinRequests() throws Exception {
		MvcResult mvcResult = mockMvc.perform(get(UriComponentsBuilder.fromPath(Endpoints.FAMILY_LIST_JOIN_REQUEST)
				...
				.andReturn();
		JoinRequestsResponse response = objectMapper.readValue(mvcResult.getResponse().getContentAsString(),
				JoinRequestsResponse.class);
	...
	public void shouldResendInvite() throws Exception {
		...
		InvitationResponse response = objectMapper.readValue(mvcResult.getResponse().getContentAsString(),
		...
		mockMvc.perform(
						post(UriComponentsBuilder.fromPath(Endpoints.RESEND_INVITATION).buildAndExpand(invitation.getId())
								.toUriString())
	...
	public void shouldRevokeInvite() throws Exception {
		MvcResult mvcResult = mockMvc.perform(
						post(UriComponentsBuilder.fromPath(Endpoints.REVOKE_INVITATION).buildAndExpand(invitationId)
								.toUriString())
								.with(oidcLogin().oidcUser(
										getOidcUser(USER_ID, List.of("SCOPE_ExpenseManager.Family.Member.INVITE")))))
				.andExpect(status().isOk()).andReturn();
