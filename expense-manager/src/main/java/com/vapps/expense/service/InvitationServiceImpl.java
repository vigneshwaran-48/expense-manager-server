...
public class InvitationServiceImpl implements InvitationService {
	...
	public boolean isMemberInvitedToFamily(String userId, String memberId) throws AppException {
		...
		if (family.isEmpty()) {
			throw new AppException(HttpStatus.BAD_REQUEST.value(),
					"You are not in a family to search non invited users!");
		}
	...
	private void checkDuplicateInvitation(String userId, InvitationDTO invitation) throws AppException {
		if (invitationRepository.findByRecipientIdAndFromIdAndType(invitation.getRecipient().getId(), userId,
			...
			LOGGER.info("Invitation {} is already present",
					invitationRepository.findByRecipientIdAndFromIdAndType(invitation.getRecipient().getId(), userId,
							invitation.getType()).get().getId());
